...
###########################################################################################################
#Loading the genetic data and preparing the datafile for other software/packages
###########################################################################################################
...
#a function for exporting the file to a "STRUCTURE" file format, this function is only working with the 
#appropriate input datafile (ie a file format similar to "BRAdata.txt")
...
#only a few edition to the output file are needed before running STRUCTURE on it (remove "", and the two 
...
#### TESS file format
#a function for exporting the file to a "TESS" file format, this function is only working with the 
#appropriate input datafile (ie a file format similar to "BRAdata.txt")
tessexport<-function(fileinput) {
  temp<-fileinput[,c(1:2,4:5,14:27)]
  #we add some noise to geographic coordinates, so each individual has different coordinates
...
tessexport(BRA)
#remove "" in the output datafile, then change the name of the 'output.tess' file before running the 
#function again, or else the file will be overwrite
tessexport(BRAcccons)
tessexport(BRAcc)

#### GENELAND file format
#a function for exporting the file to "GENELAND" file format, this function is only working with the 
#appropriate input datafile (ie a file format similar to "BRAdata.txt"), it is only necessary if you 
#want to use the graphical user interface of Geneland ('Geneland.GUI()')
genelandexport<-function(fileinput) {
  ...
  write.table(tempgeo,file="outputgeo.geneland",row.names=F,col.names=FALSE,quote=FALSE,sep=" ")
  write.table(tempgen,file="outputgen.geneland",row.names=F,col.names=FALSE,quote=FALSE,sep=" ")
}

genelandexport(BRA)
#Change the name of the 'outputgeo.geneland' and 'outputgen.geneland' files before running the 
#function again, or else the file will be overwrite
...
#iterations followed by 500.000 simulations. Fifteen repetitions of each run were performed for 
...
municipioSH<-readShapePoly("mucipio_MT.shp", proj4string=CRS("+proj=longlat +datum=WGS84"))
voies<-readShapeLines("voies.shp", proj4string=CRS("+proj=longlat +datum=WGS84"))

#shapefile of the Hevea brasiliensis plantations identified by analysis of satellite images
parcPoly<-readShapePoly("parc_MT.shp", proj4string=CRS("+proj=longlat +datum=WGS84"))
#in order to have the area of the different plantations in square meters and the distance 
#in meters, we turn the coordinates of the object into planar coordinates format
...
parcPoints<-readShapePoints("parc_point.shp", proj4string=CRS("+proj=longlat +datum=WGS84"))
#in order to have the distance in meters, we turn the coordinates of the object into planar coordinates format
parcPoints.utm <- spTransform(parcPoints, CRS("+proj=utm +zone=21 +datum=WGS84"))
sampPoints<-readShapePoints("prelevement.shp", proj4string=CRS("+proj=longlat +datum=WGS84"))
...
#in order to have the distance in meters, we turn the coordinates of the object into planar coordinates format
sampPoints.utm <- spTransform(sampPoints, CRS("+proj=utm +zone=21 +datum=WGS84"))

#loading the altitudinal data, no need to clip the information, because it has been already clipped
alt<-raster("alt_Clip1.img")

#loading the raster of the land cover, no need to clip the information, because it has been already clipped
...
extract(veget,temp,cellnumbers=TRUE)[1] #determining the first cell of the grid we want to modify
...
site_table<-data.frame("site_ID"=sampPoints$name_id,coordinates(sampPoints)[,1:2],
                       "Altitude"=extract(alt,coordinates(sampPoints)[,1:2]), 
                       "BioClim1"=extract(bioclim1,coordinates(sampPoints)[,1:2])/10,
                       "BioClim2"=extract(bioclim2,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim3"=extract(bioclim3,coordinates(sampPoints)[,1:2]), 
                       "BioClim4"=extract(bioclim4,coordinates(sampPoints)[,1:2])/100, 
                       "BioClim5"=extract(bioclim5,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim6"=extract(bioclim6,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim7"=extract(bioclim7,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim8"=extract(bioclim8,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim9"=extract(bioclim9,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim10"=extract(bioclim10,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim11"=extract(bioclim11,coordinates(sampPoints)[,1:2])/10, 
                       "BioClim12"=extract(bioclim12,coordinates(sampPoints)[,1:2]), 
                       "BioClim13"=extract(bioclim13,coordinates(sampPoints)[,1:2]), 
                       "BioClim14"=extract(bioclim14,coordinates(sampPoints)[,1:2]), 
                       "BioClim15"=extract(bioclim15,coordinates(sampPoints)[,1:2]), 
                       "BioClim16"=extract(bioclim16,coordinates(sampPoints)[,1:2]), 
                       "BioClim17"=extract(bioclim17,coordinates(sampPoints)[,1:2]), 
                       "BioClim18"=extract(bioclim18,coordinates(sampPoints)[,1:2]), 
                       "BioClim19"=extract(bioclim19,coordinates(sampPoints)[,1:2]), 
                       stringsAsFactors = FALSE)
colnames(site_table)<-c("site_ID","Longitude", "Latitude", "Altitude", "Annual Mean Temperature", 
                        "Mean Diurnal Range (Mean of monthly (max temp - min temp))", 
                        "Isothermality", "Temperature Seasonality (standard deviation *100)", 
...
#plot(parcPoly,add=TRUE, col="red") #not very usefull because area of plantation are 
...
plot(parcPoints,add=TRUE, col="black",bg="blue",pch=21)
plot(sampPoints,add=TRUE, col="black",bg="red",pch=21)
polygon(x=c(-53.39,-53.13,-53.13,-53.39,-53.39),y=c(-13.29,-13.29,-13.03,-13.03,-13.29),col="white",
        density=35,border="white",lwd=1.5)
box()
par(op)
#the following code is adapted from http://www.stat.auckland.ac.nz/~paul/RGraphics/examples-map.R
#plotting an inset map of south america
maplocs <- map("worldHires", ylim=c(-60,12),xlim=c(-89,-35), col="lightblue1",fill=TRUE,add=TRUE,plot=FALSE)
...
par(fig=c(0.93 - 0.15, 0.929, 0.72, 0.72 + 0.15*aspect), mar=rep(0, 4), new=TRUE, pty="s")
plot.new()
plot.window(xlim=xrange, ylim=yrange)
polygon(x=c(-88.5,-34.5,-34.5,-88.5,-88.5),y=c(-58,-58,15,15,-58),col="lightblue1")
map("worldHires", ylim=c(-60,12),xlim=c(-89,-35), col="white",fill=T,add=TRUE)
polygon(x=c(-59.5,-52.5,-52.5,-59.5,-59.5),y=c(-17.5,-17.5,-10.5,-10.5,-17.5),col="grey")
#plotting an inset map of a zoom on a part of Mato Grosso plantations
par(fig=c(0.93 - 0.15, 0.9298, 0.4, 0.4 + 0.15), mar=rep(0, 4), new=TRUE, pty="s")
plot.new()
plot.window(xlim=c(-53.39,-53.13), ylim=c(-13.29,-13.03))
polygon(x=c(-53.39,-53.13,-53.13,-53.39,-53.39),y=c(-13.29,-13.29,-13.03,-13.03,-13.29),col="white")
plot(parcPoly,col="red",xlim=c(-53.39,-53.13),ylim=c(-13.29,-13.03),lwd="0.2",add=TRUE)
#export in pdf format with a size of 9.5*9.5 inches in order to obtain the same figure


map("world",regions=c("brazil","argentina","uruguay","paraguay","chile","bolivia",
                      "peru","ecuador","colombia","venezuela","guyana","suriname",
                      "french guiana","panama","costa rica","nicaragua"))
...
#first we built the combined table of coordinates of plantations and sampling sites
xy.utm<-rbind(coordinates(parcPoints.utm),coordinates(sampPoints.utm)[,c(1,2)])
row.names(xy.utm)[577:591]<-c("BBU","DEN","DAQ1","DAQ2","DAQ3","NHO","PEM1","PEM2",
...
#Distances from the Minimum spanning tree network between Hevea brasiliensis plantation
MinSpaTre.utm<-spantree(matdist.utm) #we use this function to find the minimum spanning tree
#it is then possible to compute the distances between the nodes of the tree, following 
#a tree path
...
#we ordered this table to match the organization of the genetic table
dist_MinSpanTree<-dist_MinSpanTree[order(colnames(dist_MinSpanTree)),order(colnames(dist_MinSpanTree))]

#Distances between sampling point using roads. This distances has been computed using google maps
#https://maps.google.fi/maps?hl=en&tab=wl in July 2013 by copying and pasting coordinates of the 
#plantation
dist_roads<-as.matrix(read.table("dist_roads.txt",header=TRUE))

#in order to plot the tree on the map, we built another tree with WGS84 coordinates
...
MinSpaTre<-spantree(matdist) #we use this function to find the minimum spanning tree
...
# scatterutil.eti(sampPoints$POINT_X,sampPoints$POINT_Y,as.character(sampPoints$name_id),0.5)
# #or a simplier way
...
MCMC(coordinates=BRAcc[,c(4,5)], geno.hap=BRAcc[,c(14:27)], varnpop=TRUE, npopmax=15,spatial=TRUE,
     freq.model="Uncorrelated", nit=100000, thinning=100, path.mcmc="c:/Users/bbarres/GENELAND/BRA/",
     filter.null.alleles=FALSE,delta.coord=0.003,npopinit=2)

PostProcessChain(coordinates=BRAcc[,c(4,5)], path.mcmc="c:/Users/bbarres/GENELAND/BRA/",
                 nxdom=100,nydom=100, burnin=200)

Plotnpop(path.mcmc="c:/Users/bbarres/GENELAND/BRA/",burnin=200)

PlotTessellation(coordinates=BRAcc[,c(4,5)], path.mcmc="c:/Users/bbarres/GENELAND/BRA/")

PosteriorMode(coordinates=BRAcc[,c(4,5)], path.mcmc="c:/Users/bbarres/GENELAND/BRA/")
...
PrivAllRich<-function(data)
{
  ...
  for (i in 1:(dim(datapop@tab)[1])) {
    ...
    ifelse(dim(temp)[2]==1,CoMat3[i,]<-apply(temp,1,prod),CoMat3[i,]<-apply(temp,2,prod))
  }
  CoMat4<-CoMat*CoMat3
...
boxplot(distri[[1]][,1], distri[[1]][,2], main="Comparison of Allelic Richness",
...
boxplot(distri[[2]][,1], distri[[2]][,2], main="Comparison of Private Allelic Richness",
        names=c(colnames(distri[[1]])[1],colnames(distri[[1]])[2]))
...
boxplot(distri[[3]][,2], distri[[3]][,1], main="Comparison of Nei Diversity", 
...
#recovering the area information for each identified plantations in square km (reason why we 
#divided by 1000000)
plant_area<-(sapply(slot(parcPoly.utm, "polygons"), slot, "area"))/1000000
#we add the area of the sampling sites to the table of area
#since the surface on which the samples were collected was limited, we fixe this area to 1 ha
...
alpha<-1
racArea<-sqrt(plant_area) #conversion of area in square meter to the square root of the area in 
#square km
